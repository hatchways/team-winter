...
def get_stored_credentials(user_id):
    ...
    return session['gmail_credentials']
...
def store_credentials(user_id, credentials):
    ...
    # Store credentials in the session for now
...
def get_credentials(authorization_code, state):
    ...
    try:
        ...
        e.authorization_url = get_authorization_url(email_address, state)
        raise e
...
def login():
    ...
    return redirect(get_authorization_url(email, None))

@bp.route('/authorize', methods=['GET'])
def authorize():
    code = request.args.get('code')
    return get_credentials(code, None)
